...
def crawl(base_url: str, num_pages: int, seed_path: str, crawl_output_path: str, start: int = 2, timer: bool = False):
    start_time = perf_counter()
    generate_seed_file(base_url, num_pages, seed_path, start)
    generate_crawl_data(crawl_output_path, seed_file=seed_path)
...
def generate_seed_file(base_url: str, num_pages: int, seed_path: str, start: int = 2):
...
def read_seed_file(seed_path: str) -> list:
    seed_data = []
...
def generate_crawl_data(crawl_output_path: str, seed_data: list = None, seed_file: str = None):
    from py.util import ALZConnectedScraper
    ...
    with open(seed_file, 'r') as sf:
        ...
        for line in sf:
            ...
            for link in discussion_links:
                save_crawled_data(crawl_output_path, link)

# save one by one (appending to end of file), to avoid holding all data in memory
def save_crawled_data(crawl_output_path: str, url: str):
